...
export default function applyReactPluginToHTML(
	...
	// Prefer package names (resolved from the app cwd). Skip hard-coded
	// node_modules paths so temp-dir integration fixtures still build.
		...
		build: {
			buildConfig: () => ({
				...
				files: {
					...
					"@apply-react/props.ts": `const props = ${JSON.stringify({ ...props, hydration, entrypointExtensions, fallbacks, enableHMR })}; export default props;`,
